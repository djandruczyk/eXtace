...
int open_sound(DataSource source)
{
  int i=0;
...
int audio_thread_starter(int i)
{
  int err = -1;  
#ifdef HAVE_COMEDI
  int ret;
  ...
  if(i<0 || i>=MAX_HANDLES || !handles[i].opened)
    {
      fprintf(stderr,__FILE__": Invalid handle %i in audio_thread_starter, "
	      " opened=%i\n",i,handles[i].opened);
      return -1;
    }
...
#ifdef HAVE_ESD
    case ESD:
      err = pthread_create(&(handles[i].sound_thread),
...
int audio_thread_stopper(int i)
{
  int err = -1;
  ...
  switch (handles[i].source)
    ...
    case ESD:
      ...
      err=0;
#else
      err=pthread_cancel(handles[i].sound_thread);
      if(err == ESRCH)
